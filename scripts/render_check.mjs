...
async function exercise(viewport, name, deviceScaleFactor = 1) {
  ...
  await publishing.hover();
  const publishingActive = await publishing.evaluate((el) => el.classList.contains('is-current'));
  if (!publishingActive) throw new Error(`${name}: floor-plan room interaction did not activate`);

  await page.screenshot({ path: `${out}/catalogue-${name}.png`, fullPage: true });
...
await reducedPage.locator('[data-enter-house]').last().focus();
