...
async function exercise(viewport, name, deviceScaleFactor = 1) {
  ...
  await assertWithinViewport(page, ['.arrival-copy', '.arrival-kicker', '.arrival h1', '.arrival-story', '.threshold-cta', '.house-stage', '.arrival-foot'], `${name} homepage`);
  ...
  await assertWithinViewport(page, ['.interior-copy', '.interior-copy h1', '.interior-copy>p', '.plan-theatre'], `${name} catalogue`);
  ...
  await assertWithinViewport(page, ['#business .chapter-grid', '#business .chapter-copy', '#business h2'], `${name} business room`);
  await page.screenshot({ path: `${out}/room-business-${name}.png`, fullPage: false });

  await page.locator('#publishing').scrollIntoViewIfNeeded();
  await page.waitForTimeout(80);
  await assertWithinViewport(page, ['#publishing .chapter-grid', '#publishing .chapter-copy', '#publishing h2'], `${name} publishing room`);
  ...
  await page.waitForTimeout(80);
  await assertWithinViewport(page, ['.desk-grid', '.desk-copy', '.contact-form'], `${name} front desk`);
...
console.log('PASS: browser interaction, responsive, viewport-bounds and reduced-motion checks');
